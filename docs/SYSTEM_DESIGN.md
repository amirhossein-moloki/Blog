...
classDiagram
    ...
    subgraph ArticlesDomain ["Articles Domain"]
        class AuthorProfile {
            +string display_name
            +text bio
        }
...
```mermaid
flowchart TD
    Start[Article Saved] --> Sync[sync_article_media service]
    ...
    Find --> Update[Update ArticleMedia through model]
...
sequenceDiagram
    ...
    participant Service as sync_article_media
    participant DB as Database

    Author->>API: POST /api/articles/ (content, images)
    API->>DB: Save Article instance
    API->>Service: Trigger sync_article_media(article)
    Service->>DB: Query Media by storage_key
    Service->>DB: Create/Delete ArticleMedia links
    DB-->>Service: Success
    Service-->>API: Done
...
- **Service Layer**: Encapsulates business logic (e.g., `sync_article_media`, `create_comment`) to keep views thin.
...
flowchart TD
    UsersApp --> CoreApp
    ArticlesApp --> UsersApp
    ArticlesApp --> MediasApp
    InteractionsApp --> UsersApp
    InteractionsApp --> ArticlesApp
...
- **Tight Coupling**: `Articles` and `Medias` are relatively tightly coupled due to the `sync_article_media` logic.
...
- **Bottlenecks**: Heavy HTML parsing in `sync_article_media` for large posts.
