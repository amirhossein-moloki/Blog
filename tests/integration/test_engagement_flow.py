...
class EngagementFlowIntegrationTest(BaseAPITestCase):
    ...
    def test_commenting_and_reply_flow(self, mock_notify):
        ...
        self.assertTrue(Comment.objects.filter(id=comment_id, user=self.user, post=post).exists())
        ...
        self.assertTrue(Comment.objects.filter(id=reply_id, parent_id=comment_id).exists())
    ...
    def test_reaction_on_post(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertTrue(Reaction.objects.filter(user=self.user, object_id=post.id, reaction='like').exists())
    ...
    def test_reaction_on_comment(self):
        ...
        response = self.client.post(url, reaction_data, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertTrue(Reaction.objects.filter(user=self.user, content_type=content_type, object_id=comment.id).exists())
