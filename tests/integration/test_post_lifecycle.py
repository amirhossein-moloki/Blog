...
from posts.factories import PostFactory, AuthorProfileFactory, MediaFactory
...
class PostLifecycleIntegrationTest(BaseAPITestCase):
    def test_post_creation_and_media_sync(self):
        ...
        attachments = PostMedia.objects.filter(post=post, media=media, attachment_type='in-content')
    ...
    def test_post_creation_with_cover_and_og_image(self):
        ...
        self.assertTrue(PostMedia.objects.filter(post=post, media=cover, attachment_type='cover').exists())
        self.assertTrue(PostMedia.objects.filter(post=post, media=og_image, attachment_type='og-image').exists())
