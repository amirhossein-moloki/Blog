...
def sync_post_media(post):
    post.media_attachments.filter(attachment_type='cover').exclude(media=post.cover_media).delete()
    if post.cover_media:
        post.media_attachments.update_or_create(media=post.cover_media, defaults={'attachment_type': 'cover'})

    post.media_attachments.filter(attachment_type='og-image').exclude(media=post.og_image).delete()
    if post.og_image:
        post.media_attachments.update_or_create(media=post.og_image, defaults={'attachment_type': 'og-image'})
    ...
    current_media_ids = set(
        post.media_attachments.filter(attachment_type='in-content').values_list('media_id', flat=True)
    )
    ...
    post.media_attachments.filter(media_id__in=ids_to_remove, attachment_type='in-content').delete()
