...
from posts.services import increment_post_view_count, publish_scheduled_posts, sync_post_media
from posts.factories import PostFactory, AuthorProfileFactory
...
class PostServiceTests(TestCase):
    ...
    def test_increment_post_view_count_error(self):
        ...
        with self.assertLogs('posts.services', level='ERROR') as cm:
            with patch('posts.models.Post.objects.filter', side_effect=Exception("DB Error")):
    ...
    def test_sync_post_media_cover_and_og(self):
        post = PostFactory()
        media_cover = Media.objects.create(storage_key="cover.jpg", url="http://ex.com/cover.jpg", type="image", mime="image/jpeg")
        media_og = Media.objects.create(storage_key="og.jpg", url="http://ex.com/og.jpg", type="image", mime="image/jpeg")

        post.cover_media = media_cover
        post.og_image = media_og
        sync_post_media(post)

        self.assertTrue(PostMedia.objects.filter(post=post, media=media_cover, attachment_type='cover').exists())
        self.assertTrue(PostMedia.objects.filter(post=post, media=media_og, attachment_type='og-image').exists())

        # Remove them
        post.cover_media = None
        post.og_image = None
        sync_post_media(post)
        self.assertFalse(PostMedia.objects.filter(post=post, media=media_cover, attachment_type='cover').exists())
        self.assertFalse(PostMedia.objects.filter(post=post, media=media_og, attachment_type='og-image').exists())

    def test_sync_post_media_in_content(self):
        with self.settings(MEDIA_URL='/media/'):
            media = Media.objects.create(storage_key="content.jpg", url="/media/content.jpg", type="image", mime="image/jpeg")
            post = PostFactory(content='<img src="/media/content.jpg">')
            # Post.save calls sync_post_media

            self.assertTrue(PostMedia.objects.filter(post=post, media=media, attachment_type='in-content').exists())

            # Change content
            post.content = 'no image'
            sync_post_media(post)
            self.assertFalse(PostMedia.objects.filter(post=post, media=media, attachment_type='in-content').exists())
