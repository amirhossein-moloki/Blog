...
from posts.models import Post, AuthorProfile, Category, Tag, Revision
...
class PostViewSetTests(APITestCase):
    def setUp(self):
        ...
        self.author_profile = AuthorProfile.objects.create(user=self.user, display_name="Author")
    ...
    def test_get_queryset_authenticated_filter(self):
        ...
        other_author = AuthorProfile.objects.create(user=other_user, display_name="Other")
        Post.objects.create(title="Other Draft", slug="other-draft", author=other_author, status='draft')
    ...
    def test_similar_posts(self):
        ...
        Post.objects.create(
            title="Similar Post", slug="similar-post", author=self.author_profile,
            category=self.category, status='published', published_at=timezone.now()
        )
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_similar_posts_no_category(self):
        post_no_cat = Post.objects.create(title="No Cat", slug="no-cat", author=self.author_profile, status='published')
    ...
    def test_same_category_no_category(self):
        post_no_cat = Post.objects.create(title="No Cat 2", slug="no-cat-2", author=self.author_profile, status='published')
    ...
    def test_publish_post_api(self):
        draft = Post.objects.create(title="Draft", slug="draft-post", author=self.author_profile, status='draft')
        self.client.force_authenticate(user=self.user)
    ...
    def test_publish_post_api_unauthorized(self):
        ...
        draft = Post.objects.create(title="Draft 2", slug="draft-post-2", author=self.author_profile, status='draft')
    ...
    def test_publish_post_api_invalid_status(self):
        self.client.force_authenticate(user=self.user)
        url = reverse('posts:post-publish', kwargs={'slug': self.post.slug}) # Already published
        response = self.client.post(url)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

    def test_related_posts_no_tags(self):
        post_no_tags = Post.objects.create(title="No Tags", slug="no-tags", author=self.author_profile, status='published')
...
class PostSerializerTests(APITestCase):
    def setUp(self):
        ...
        self.author_profile = AuthorProfile.objects.create(user=self.user, display_name="Author 2")
        self.admin = User.objects.create_superuser(username='admin2', password='password', email='admin2@example.com')
    ...
    def test_ckeditor_upload_view_not_image(self):
        from django.core.files.uploadedfile import SimpleUploadedFile
        uploaded_file = SimpleUploadedFile("test.txt", b"not image", content_type="text/plain")
        url = reverse('ckeditor_upload')
        self.client.force_login(user=self.admin)
        response = self.client.post(url, {'upload': uploaded_file}, format='multipart')
        self.assertEqual(response.status_code, 400)
    ...
    def test_ckeditor_upload_view_unauthorized(self):
        user_no_profile = User.objects.create_user(username='noprofile2', password='password')
        url = reverse('ckeditor_upload')
