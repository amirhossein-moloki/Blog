...
## ویژگی‌های اصلی
*   **مدیریت هویت:** احراز هویت مبتنی بر Static API Key و کنترل دسترسی مبتنی بر نقش (RBAC).
...
**هدف کسب‌و‌کار:** مدیریت حساب‌های کاربری، جریان‌های احراز هویت و کنترل‌های دسترسی.
...
*   **urls.py:** تعریف مسیرها برای `users/`.
...
| آدرس URL | متد | اکشن ViewSet | توضیحات |
| :--- | :--- | :--- | :--- |
...
# بخش ۷ — احراز هویت و تعیین سطح دسترسی

## جریان احراز هویت
سیستم از **احراز هویت API Key ثابت** استفاده می‌کند.
1.  تمام درخواست‌های API باید شامل هدر `X-API-Key` با یک کلید معتبر باشند.
2.  به صورت اختیاری، می‌توان از هدر `X-Test-User` در محیط توسعه برای شناسایی کاربر استفاده کرد.
