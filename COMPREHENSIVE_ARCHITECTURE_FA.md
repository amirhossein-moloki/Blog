...
## ویژگی‌های اصلی
*   **مدیریت هویت:** احراز هویت مبتنی بر JWT، یکپارچگی با Google OAuth2 و کنترل دسترسی مبتنی بر نقش (RBAC).
...
*   **کتابخانه رسانه:** ثبت مرکزی رسانه‌ها با تبدیل خودکار تصاویر به AVIF و بهینه‌سازی ویدیو بر پایه FFmpeg.
...
*   **common/fields.py:** `OptimizedImageField` / `OptimizedVideoField` برای پردازش خودکار.
...
*   **common/utils/images.py:** منطق اصلی تبدیل AVIF با استفاده از Pillow.
*   **common/tasks.py:** `convert_image_to_avif_task` برای بهینه‌سازی در پس‌زمینه.
...
5.  **بهینه‌سازی:** `convert_image_to_avif_task` برای هر آپلود مستقیم تصویر در صف قرار می‌گیرد.
6.  **زمان‌بندی:** اگر `status='scheduled'` باشد، Celery Beat در نهایت `publish_scheduled_posts_task` را برای عمومی کردن آن اجرا می‌کند.
...
۲.  **تبدیل AVIF:** در `create_media_from_file` برای تمام آپلودهای `image/*` فعال می‌شود (کیفیت ۸۵٪).
۳.  **انتشار زمان‌بندی شده:** توسط وظیفه Celery هر ۶۰ ثانیه مدیریت می‌شود؛ بررسی می‌کند `scheduled_at <= now`.
۴.  **مشاهده‌پذیری پست:** کاربران عادی (`IsAuthenticatedOrReadOnly`) فقط می‌توانند پست‌هایی با `status='published'` را کوئری کنند.
۵.  **شمارش بازدید:** اکشن `retrieve` در `PostViewSet` تعداد `views_count` را با استفاده از عبارات `F()` افزایش می‌دهد تا از تداخل (race conditions) جلوگیری شود.
...
## نمودار فعالیت: آپلود و بهینه‌سازی رسانه

```mermaid
stateDiagram-v2
    [*] --> آپلود: فایل دریافت شد
    آپلود --> بررسی_نوع: آیا تصویر است؟
    بررسی_نوع --> تبدیل_AVIF: بله
    بررسی_نوع --> ذخیره_اصلی: خیر
    تبدیل_AVIF --> استخراج_متادیتا
    ذخیره_اصلی --> استخراج_متادیتا
