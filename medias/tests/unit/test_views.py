...
class MediaViewSetTests(APITestCase):
    ...
    def test_create_media_file(self):
        ...
        uploaded_file = SimpleUploadedFile("test.mp4", b"hello world", content_type="video/mp4")
    ...
    def test_get_queryset_and_serializer_class(self):
        ...
        # Retrieve
        media = Media.objects.create(
            storage_key='test2.txt', url='/media/test2.txt', size_bytes=10,
            mime='text/plain', title='Test 2', uploaded_by=self.user, type='file'
        )
...
class MediaServiceExtraTests(APITestCase):
    ...
    def test_create_media_video(self):
        ...
        uploaded_file = SimpleUploadedFile("test.mp4", b"fake video content", content_type="video/mp4")
    ...
    def test_create_media_generic_file(self):
        from medias.services import create_media_from_file
        uploaded_file = SimpleUploadedFile("test.pdf", b"fake pdf content", content_type="application/pdf")
        media = create_media_from_file(uploaded_file, self.user)
    ...
    def test_create_media_image_corrupt(self):
        from medias.services import create_media_from_file
        uploaded_file = SimpleUploadedFile("corrupt.jpg", b"not an image", content_type="image/jpeg")
