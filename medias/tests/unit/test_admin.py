...
class MediaAdminTests(TestCase):
    def setUp(self):
        self.site = AdminSite()
        self.user = User.objects.create_superuser(username='admin3', password='password', email='admin3@example.com')
        self.media = Media.objects.create(
            storage_key='test.txt', url='/media/test.txt', size_bytes=10,
            mime='text/plain', title='Test', uploaded_by=self.user, type='file'
        )
