...
from posts.factories import ArticleFactory, UserFactory
...
class InteractionServicesTest(TestCase):
    ...
    def test_create_comment_service(self, mock_notify):
        ...
        create_comment(user=user, article=article, content="Test content")
    ...
    def test_toggle_reaction(self):
        user = UserFactory()
        article = ArticleFactory()
        reaction = toggle_reaction(user, article, "like")
        self.assertIsNotNone(reaction)
        self.assertEqual(Reaction.objects.count(), 1)

        toggle_reaction(user, article, "like")
