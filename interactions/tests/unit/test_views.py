...
from posts.models import Post, AuthorProfile
...
class InteractionsViewSetTests(APITestCase):
    def setUp(self):
        ...
        self.author_profile = AuthorProfile.objects.create(user=self.admin, display_name="Admin Author")
    ...
    def test_reaction_queryset_staff(self):
        ...
        Reaction.objects.create(user=self.user, content_type=ct, object_id=self.post.id, reaction='like')
        self.client.force_authenticate(user=self.admin)
    ...
    def test_reaction_queryset_regular(self):
        ...
        ct = ContentType.objects.get_for_model(Post)
        Reaction.objects.create(user=self.user, content_type=ct, object_id=self.post.id, reaction='like')
        Reaction.objects.create(user=other_user, content_type=ct, object_id=self.post.id, reaction='like')
